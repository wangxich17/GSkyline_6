...
GGroup PWiseP::PointWiseInit(int k, vector<GPoint *> PointCell) {
    ...
    for (int i = PointCell.size() - 1; i >= 0; i--) {
        ...
        if (p_layer == 0) {//最顶层
            ...
            GPoint *parent = NULL;
            for (vector<GPoint *>::iterator it = p->ParentSet.begin();
                 it != p->ParentSet.end(); it++) {//获取FirstParent并将p放入对应的CS
...
void PWiseP::Solve(int k, GGroup &g) {
    ...
    for (list<GPoint *>::iterator it = (lastPoint->SimpleCSet).begin();
         it != lastPoint->SimpleCSet.end(); it++) {//g的尾节点的子节点集
